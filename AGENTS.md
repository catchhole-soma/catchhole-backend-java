...
- 운영 AI 모델과 추론 강도는 `LLM_MODEL`, `LLM_REASONING_EFFORT`로, 신규 회원 기본 지급량은 `AI_TOKEN_DEFAULT_GRANT`로 주입한다. 실제 값은 `/opt/catchhole/.env`에만 두고 Compose가 Backend와 AI Worker에 명시적으로 전달한다.
