...
- 후보 응답의 `valueValidation`(`VALID`, `INVALID`, `NOT_APPLICABLE`)은 현재 활성 schema와 저장된 값에서 조회 시점에 파생하고 DB에 저장하지 않는다. 기존 후보는 backfill하지 않고, `INVALID`는 수정·무시는 허용하되 2차 비교와 단건·그룹 확정 전에 fail-closed로 거절한다. 레거시 이상 데이터를 숨기지 않고 화면에서 교정할 수 있게 하기 위한 정책이다.
