...
- 캐릭터 상세 설정의 편집 계약은 활성 schema로 해석한다. exact key는 key와 표시명을 모두 잠그고, pattern key는 응답의 `attributeNamePrefix`를 보존한 suffix만 수정하며, `manual_`·미등록 custom key는 key를 유지한 채 표시명만 수정한다. pattern suffix의 공백은 저장 key에서 underscore로 정규화하고 화면 표시명과 새 `valueJson.name`은 underscore를 공백으로 바꾼 값을 사용한다.
