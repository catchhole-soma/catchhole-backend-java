...
- `SettingCandidate` 확정/무시는 POST action API로 처리한다. 처음 `CONFIRMED`로 전환되는 `SETTING` 후보는 `CharacterFact`를 항상 새 이력으로 저장하고, 기본 `APPLY_PROPOSAL`은 완료된 비교 제안을 `WorkCharacter` snapshot과 provenance에 반영한다. 사용자가 `HISTORY_ONLY`를 선택하면 Fact만 저장하고 snapshot·provenance·snapshotVersion은 바꾸지 않는다. 비교 Worker가 캐릭터 후보를 `EXCLUDE`로 완료하면 같은 트랜잭션에서 `SettingCandidate.dismiss()`를 호출해 `DISMISSED + NOT_REQUIRED`로 자동 전환하고 Fact·snapshot·provenance·이력을 만들지 않는다. 같은 완료 요청은 멱등 처리하며 기존 `PENDING_REVIEW + COMPLETED + EXCLUDE`는 forward migration으로 같은 상태에 수렴시킨다. `REVIEW_REQUIRED`는 제안 적용 확정을 막되 이력 저장은 허용한다. 이 자동 무시 정책은 세계관 후보에는 적용하지 않는다. `CHARACTER_DISCOVERY` 후보는 캐릭터와 최초 등장만 반영하고 Fact를 만들지 않는다.
