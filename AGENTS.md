...
- 후보 응답의 `valueValidation`(`VALID`, `INVALID`, `NOT_APPLICABLE`)은 현재 활성 schema와 저장된 값에서 조회 시점에 파생하고 DB에 저장하지 않는다. 값 계약 오류는 `repairable=true`로 수정·무시를 허용하고, schema 없음·모호성·타입 불일치는 `repairable=false`로 구분해 현재 수정 API로 고칠 수 있다고 안내하지 않는다. 두 종류 모두 2차 비교와 단건·그룹 확정 전에 fail-closed로 거절한다.
