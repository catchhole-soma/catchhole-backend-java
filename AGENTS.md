...
`docs/gh199-late-review-and-private-images.md`를 따른다. 완료된 순차 후보의 늦은 확정은 최신값·제거 기록·직접 수정을 보호하고 충돌은 이력에만 남긴다. 세계관의 `historyOnly` 후보를 현재값 근거로 재사용하지 않는다. 후보 변경만으로 후속 Job을 무효화하거나 LLM을 추가 호출하지 않는다. V67 계정 이미지/CHI1 호환과 V68 세계관 이력·수동 경로 표시를 유지한다. 기존 CHI1 문서 중 새 업로드에도 키를 요구하는 내용은 이 정책으로 대체한다.
