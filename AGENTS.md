...
- 일반 `SETTING_EXTRACTION` Job은 `CHUNKS_READY → CHARACTER_CANDIDATES_SAVED → CHARACTER_COMPARISONS_HANDED_OFF → WORLD_CANDIDATES_PUBLISHED → WORLD_COMPARISONS_FINISHED` checkpoint를 단조 증가시킨다. `CHARACTER_COMPARISONS_HANDED_OFF`는 회차별 후보 게시 책임을 그룹 숨김 Job에 넘겼다는 뜻이며 비교 완료로 재해석하지 않는다. 같은 batch의 모든 원 Job이 후보를 게시했거나 더 이상 게시할 수 없는 상태가 된 뒤에만 그룹 Job을 예약하므로 원 Worker slot은 입력 barrier를 기다리지 않는다. 구 Worker의 `CHARACTER_COMPARISONS_FINISHED`는 실제 비교 결과가 남아 있는 호환 경로로만 읽는다.
