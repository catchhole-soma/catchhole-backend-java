...
**RPO를 15분으로 정한 이유:** 원고 원문은 S3에 남고 AI 분석은 재실행할 수 있지만, 사용자가 직접 수정·추가한 캐릭터 정보와 AI 결과를 확정·삭제한 판단은 재생성할 수 없다. RDS는 트랜잭션 로그를 5분 간격으로 업로드하므로 5분 목표는 업로드·복원 가능 시점 갱신의 지연을 감안할 여유가 작고, 1시간 목표는 사용자 변경의 손실을 지나치게 크게 허용한다고 판단했다. 따라서 초기 목표를 **15분**으로 정했다. 이는 서비스의 허용 손실 기준이며 AWS의 보장값은 아니다. [AWS 로그 업로드 간격](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_PIT.html)
