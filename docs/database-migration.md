...
V14는 회원 탈퇴와 테스트 데이터 정리 시 AI 토큰 이력만 잔존해 삭제가 실패하지 않도록 V13 FK의 삭제 동작을 명시합니다.

- 회원 삭제 시 해당 회원의 계정·지급·사용 이력을 함께 삭제합니다.
- 작품 또는 분석 작업 삭제 시 연결된 요청별 사용 이력을 함께 삭제합니다.
- 운영 환경에 이미 적용된 V13을 수정하지 않고 새 migration에서 FK를 교체해 Flyway checksum을 보존합니다.
...
V1~V12 적용 DB에 현재 Backend를 시작해 V13·V14가 추가 적용되는 경로와, 빈 PostgreSQL에서 V1→V14가 순서대로 적용되는 경로를 각각 확인합니다.
