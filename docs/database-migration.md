...
├── V7__add_character_profile_setting_schemas.sql
└── V8__add_character_page_index.sql
```
...
V7은 캐릭터 상세 조회·수정에서 프로필 값을 다른 설정과 같은 `CharacterFact` 이력과 snapshot 규칙으로 관리할 수 있도록 전역 `SYSTEM_SEED` 프로필 schema 8개를 추가합니다. 테이블이나 컬럼 구조는 변경하지 않습니다.
...
V1~V6 적용 DB에 현재 Backend를 시작해 V7과 V8이 추가 적용되는 경로와, 빈 PostgreSQL에서 V1→V8이 순서대로 적용되는 경로를 각각 확인합니다.

- Flyway 로그에 V1부터 V8까지 적용 성공이 출력됩니다.
- `flyway_schema_history`에 version 1, 2, 3, 4, 5, 6, 7, 8이 성공으로 기록됩니다.
...
- Backend를 재시작해도 V1부터 V8까지 중복 적용되지 않습니다.
...
4. Backend를 시작해 Flyway V1~V8과 Hibernate validation 성공을 확인합니다.
