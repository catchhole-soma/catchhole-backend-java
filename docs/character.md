...
`characters`가 현재 화면에 보여줄 대표/현재 스냅샷이라면, `character_facts`는 나이, 레벨, 스탯, 아이템, 스킬 같은 설정이 어느 회차에서 어떤 값으로 등장했는지 추적합니다.
...
| 상태 | 의미 | 전이 시점 |
| --- | --- | --- |
| `PENDING_REVIEW` | 검토 대기 | `WorkCharacter.create()`로 캐릭터 대표 설정을 만들 때 기본값으로 설정됩니다. |
| `CONFIRMED` | 확정됨 | 사용자가 캐릭터 대표 설정을 기준 설정으로 쓰기로 하면 `WorkCharacter.confirm()`으로 전환합니다. |
| `DISMISSED` | 무시됨 | 사용자가 캐릭터 대표 설정을 쓰지 않기로 하면 `WorkCharacter.dismiss()`로 전환합니다. |

정책 미확정 TODO:

- `WorkCharacter.reviewStatus`가 실제로 필요한지, `SettingCandidate`와 `CharacterFact`의 검토 상태만으로 충분한지 후속 API 설계 때 결정합니다.
...
`CharacterFactReviewStatus`

| 상태 | 의미 | 전이 시점 |
| --- | --- | --- |
| `PENDING_REVIEW` | 검토 대기 | 개별 설정값을 `CharacterFact.create()`로 만들 때 기본값으로 설정됩니다. |
| `CONFIRMED` | 확정됨 | 사용자가 개별 설정값을 기준 설정 또는 이력으로 확정하면 `CharacterFact.confirm()`으로 전환합니다. |
| `DISMISSED` | 무시됨 | 사용자가 개별 설정값을 반영하지 않기로 하면 `CharacterFact.dismiss()`로 전환하고 `isCurrent=false`로 둡니다. |
...
| `current_age` | 현재 나이 후보/확정값 |
| `current_level` | 현재 레벨 후보/확정값 |
...
| `fact_value` | 실제 추출 또는 확정된 표시값. 예: 17, 12, 35, OWNED |
...
| `review_status` | 설정 이력 레코드 자체의 사용자 검토 상태 |
...
- `findAllByWorkCharacterIdAndReviewStatusOrderByCreatedAtDesc(characterId, reviewStatus)`
