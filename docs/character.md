...
- JSON 타입의 설정에 세부 `properties`가 없고 같은 `(factType, factKey)`의 표시값도 바뀌지 않았다면, 상세 응답의 `properties`로 복원할 수 없는 기존 raw `valueJson`만 그대로 목표 상태에 사용합니다. 대상은 `null`, non-object, 빈 object, 예약 `value` key만 있는 object입니다. 기존 object에 예약 `value`와 공개 속성이 함께 있으면 응답에서 숨긴 `value` envelope는 유지하고 요청의 공개 `properties`만 목표 상태로 재구성합니다. `name`, `level`처럼 응답 가능한 속성이 있는 object에서 빈 `properties`를 보내면 속성 제거 요청으로 보고 기존 요청 변환을 적용합니다. 표시값이 바뀌거나 raw 값을 보존할 수 없으면 요청값을 JSON으로 파싱하며 유효하지 않은 JSON은 `CHARACTER_SETTING_VALUE_INVALID / 400`으로 거절합니다.
...
- 과거 scalar Fact의 `valueJson`이 `null`이지만 같은 표시값의 수정 요청이 `{"value": ...}`로 재조립되는 경우에는 저장 표현만 다른 동일 값으로 판단해 기존 Fact와 근거를 유지합니다. 이 예외는 JSON 타입이나 추가 속성이 있는 설정에는 적용하지 않으므로 구조화 값의 실제 변경은 계속 새 수동 Fact를 만듭니다.
