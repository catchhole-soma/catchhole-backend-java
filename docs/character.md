...
현재 범위에서는 Python AI Worker가 저장한 설정 후보를 사용자가 검토할 수 있도록 Spring 조회/수정/확정/무시 API를 제공합니다.
...
확정/무시 API는 사용자의 검토 결정을 기록하는 단계입니다. 이 API는 `SettingCandidate.reviewStatus`만 변경하며, 확정 데이터를 `CharacterFact`나 `WorkCharacter`에 반영하지 않습니다.

- `PENDING_REVIEW` 후보는 `CONFIRMED` 또는 `DISMISSED`로 전환할 수 있습니다.
- 이미 `CONFIRMED`인 후보에 다시 확정을 요청하거나, 이미 `DISMISSED`인 후보에 다시 무시를 요청하면 성공으로 처리합니다.
...
| `POST` | `/api/v1/works/{workId}/setting-candidates/{candidateId}/confirm` | 설정 후보를 `CONFIRMED` 상태로 전환합니다. |
...
flowchart TD
    ...
    G --> I["reviewStatus만 변경"]

    I -. "후속 PR" .-> J["CharacterFact 저장"]
    J -. "후속 PR" .-> K["WorkCharacter 현재 스냅샷 갱신"]
```
