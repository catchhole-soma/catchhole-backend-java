...
커밋 직후 이벤트가 이전 회차 원문·업로드 원본의 모든 version과 원고 청크·미확정 후보를 정리합니다. 저장소나 DB 정리가 실패하면 요청을 `REQUESTED`로 되돌리고 Spring 스케줄러가 기본 10초 간격으로 재시도합니다. 처리 중 서버가 재시작된 요청은 15분 뒤 stale 요청으로 회수합니다. 요청이 남아 있는 회차는 이전 파생 데이터와 새 분석 결과가 섞이지 않도록 추가 수정·삭제·분석 생성을 거절합니다. 완료 요청 row는 즉시 삭제하며 작품 영구 삭제 시에는 해당 작품의 미완료 회차 정리 요청도 함께 제거합니다.
