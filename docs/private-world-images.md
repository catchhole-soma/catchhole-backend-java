...
새 업로드는 비밀키 없이 로그인과 작품 소유권으로 보호한다. API·제한·마이그레이션·배포 순서는 [GH199 정책](gh199-late-review-and-private-images.md)을 따른다. 새 파일은 검증한 PNG이며 파일명도 서버에서 처리한다. 기존 CHI1 행과 API는 보존한다. 아래는 **이전 암호화 방식의 호환 명세와 당시 검증 기록**이며 새 업로드 UX를 설명하지 않는다.
...
스케줄러는 `cleanup_attempted_at`(미시도는 `updated_at`)이 오래된 순서로 최대 20건을 선택합니다. 각 파일 정리 전에 시각을 짧은 DB 트랜잭션으로 기록하므로 저장소 오류·부분 실패·프로세스 중단이 반복되어도 뒤의 삭제 대기와 1시간 지난 업로드를 계속 처리합니다. 최근 업로드는 회수 대상에서 제외하며 기존 업로드 시도별 경로 격리는 유지합니다.
