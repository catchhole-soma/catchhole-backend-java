...
요청은 앞뒤 공백을 제거한 `content` Unicode code point 10~1,000자와 선택적 `pagePath`를 받습니다. `pagePath`는 `/`로 시작하는 255자 이하의 내부 경로이며 query(`?`)와 fragment(`#`)를 포함할 수 없습니다.
...
`GET /api/v1/feedbacks/prompt`는 활성 작품에 보관되지 않은 실제 회차가 총 3개 이상이고 기존 의견과 안내 선점 기록이 없는 회원에게 `shouldShow: true`를 반환합니다. 최신 회차 번호가 아닌 실제 회차 행을 기준으로 판단하며 GET은 기록을 변경하지 않습니다. 전체 COUNT 대신 세 번째 유효 회차의 존재만 확인하고, V56의 활성 작품·보관되지 않은 회차 부분 인덱스를 사용하도록 조회 조건을 맞춥니다.

화면이 안내를 표시할 수 있을 때 `POST /api/v1/feedbacks/prompt/claim`을 호출합니다. 서버는 회원 row를 잠그고 자격을 다시 확인한 뒤 `members.feedback_prompt_shown_at`을 저장합니다. 이번 응답이 `shouldShow: true`일 때만 안내를 표시합니다. 동시 요청·다른 기기·이후 로그인은 `false`를 받으며, 닫기나 작성 여부와 관계없이 선점 기록을 유지합니다. 의견 작성도 같은 회원 잠금을 먼저 획득하므로 이미 의견을 저장한 회원은 이후 선점할 수 없습니다. 선점 뒤 응답이 유실된 경우에도 재노출하지 않습니다.

일반 의견 보상 요청은 일반 의견과 같은 10자 최소 조건을 사용하며 사용량 소진 피드백의 35자 최소 조건은 유지합니다.
