...
`GET /api/v1/feedbacks/prompt`는 활성 작품에 보관되지 않은 실제 회차가 총 3개 이상이고 기존 의견과 안내 선점 기록이 없는 회원에게 `shouldShow: true`를 반환합니다. 최신 회차 번호가 아닌 실제 회차 행을 집계하며 GET은 기록을 변경하지 않습니다.
