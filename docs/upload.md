...
- 현재 동기 업로드는 실패 시 `UploadBatch` row도 함께 rollback될 수 있으므로, 실패 이력을 남길 필요가 있으면 batch 선커밋, 별도 트랜잭션, 비동기 처리 중 적절한 방식을 검토합니다.
...
- 실패 사유는 `upload_batches` 별도 컬럼, 파일별 `UploadFileParseStatus.FAILED`, 공통 실패 이력 테이블 중 어떤 방식이 적절한지 검토합니다.
...
- 실패 이력을 남길 트랜잭션 경계와 `UploadBatch.fail()`/`UploadFile.markFailed()` 호출 흐름 검토
