...
기존 회차 내부 checkpoint는 재사용한다. 생성 시 선택한 모드와 run 구성은 고정하며 아래 흐름은
`ORDERED_PROVISIONAL`에만 적용한다. 새 설정 추출은 반영 방식을 생략하면 자동 누적 분석을 사용하며,
단일 회차에 `MANUAL`을 명시하면 확정 설정 기반 직접 검토를 선택할 수 있다. 기존 Job의 재시도 정책은 유지한다.

```mermaid
flowchart TD
    R["명시적 ordered 생성 요청"] --> S["회차 목록·원문·S0 고정"]
    S --> C["작품 잠금 후 claim 조건 재확인"]
    C --> P["S0 + 선행 SEALED 기록 replay"]
    P --> A["현재 회차 추출·주체 해소·비교"]
    A --> V{"처리 실패 없이 모든 후보 검증?"}
    V -- "예" --> J["변경 기록 seal + Job 완료를 원자 저장"]
    J --> C
    V -- "아니오" --> F["후속 회차 대기·명시 retry"]
    U["원문·사용자 결정 변경"] --> I["영향 이후 INVALIDATED"]
    I --> N["새 명시 요청에서 시작 기준 검증"]
```

임시 대상/설정은 정식 DB를 자동 변경하지 않는다. 사용자 확정·입력 상한·실패 재개·원문 파기의
세부 계약은 [누적 분석 설계](ordered-provisional-analysis.md)를 따른다.
