...
현재 RDS의 RPO·RTO 목표, 백업 보존·삭제 보호, 계획 삭제 시 최종 스냅샷 절차와 확인 권한은 [RDS 백업·삭제 보호 정책](../docs/rds-backup-policy.md)을 따른다. RTO 2시간은 미실측 목표이며 운영 DB 복원·전환을 이 배포 문서의 일반 재배포 절차로 실행하지 않는다.
