...
@Schema(description = "회차 감지 요청 메타데이터")
public record EpisodeDetectionRequest(
        @Schema(description = "업로드 방식", example = "SINGLE_EPISODE")
        @NotNull(message = "업로드 방식은 필수입니다.")
        EpisodeUploadType uploadType,

        @Schema(description = "단일 회차 감지 시 사용자가 입력한 회차 번호", example = "159", nullable = true)
        @Min(value = 1, message = "회차 번호는 1 이상이어야 합니다.")
        Integer singleEpisodeNo,

        @Schema(description = "단일 회차 감지 시 사용자가 입력한 회차 제목", example = "운명의 실타래", nullable = true)
