...
@RequestMapping("/api/v1/works/{workId}/characters/{characterId}/image")
@Tag(name = "CharacterImage", description = "작품별 캐릭터 대표 이미지 선택")
@SecurityRequirement(name = "bearerAuth")
public class CharacterImageController {
    private final CharacterImageService service;

    @PatchMapping
    @Operation(operationId = "updateCharacterImage", summary = "대표 이미지 선택·해제", description = "이미지 선택만 변경하며 설정 내용·설정 version·분석 상태는 변경하지 않습니다.")
    @ApiResponses({
    ...
    public CommonResponse<WorldSettingImageResponse> updateCharacterImage(
            @Parameter(hidden = true) @AuthenticationPrincipal MemberPrincipal member,
            @PathVariable UUID workId,
            @PathVariable UUID characterId,
            @Valid @RequestBody CharacterImageUpdateRequest request
    ) {
        return CommonResponse.success(service.updateCharacterImage(member.memberId(), workId, characterId, request));
    }
}
