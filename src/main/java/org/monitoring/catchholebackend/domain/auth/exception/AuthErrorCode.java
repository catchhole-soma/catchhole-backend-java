...
public enum AuthErrorCode implements ResultCode {
    ...
    AUTH_EMAIL_VERIFICATION_CODE_INVALID(HttpStatus.BAD_REQUEST, "인증번호가 올바르지 않습니다."),
    AUTH_EMAIL_VERIFICATION_TOKEN_INVALID(HttpStatus.BAD_REQUEST, "유효하지 않은 이메일 인증 토큰입니다."),
    AUTH_EMAIL_VERIFICATION_TOKEN_REQUIRED(HttpStatus.BAD_REQUEST, "이메일 인증이 필요합니다."),
    AUTH_PHONE_VERIFICATION_TOKEN_REQUIRED(HttpStatus.BAD_REQUEST, "휴대폰 인증이 필요합니다."),
    AUTH_EMAIL_VERIFICATION_EMAIL_MISMATCH(HttpStatus.BAD_REQUEST, "인증한 이메일과 가입 이메일이 일치하지 않습니다."),
    AUTH_EMAIL_VERIFICATION_EXPIRED(HttpStatus.GONE, "이메일 인증이 만료되었습니다."),
    AUTH_EMAIL_VERIFICATION_RATE_LIMITED(HttpStatus.TOO_MANY_REQUESTS, "잠시 후 다시 요청해주세요."),
    AUTH_EMAIL_VERIFICATION_ATTEMPTS_EXCEEDED(HttpStatus.TOO_MANY_REQUESTS, "인증번호 입력 가능 횟수를 초과했습니다."),
    AUTH_EMAIL_VERIFICATION_UNAVAILABLE(HttpStatus.SERVICE_UNAVAILABLE, "이메일 인증을 일시적으로 사용할 수 없습니다."),
    AUTH_SIGNUP_VERIFICATION_METHOD_DISABLED(HttpStatus.CONFLICT, "현재 사용하지 않는 회원가입 인증 방식입니다."),
