...
public class AuthController {
    ...
    @Operation(
            operationId = "signup",
            summary = "회원가입",
            description = "이메일, 비밀번호, 표시 이름과 서버가 선택한 EMAIL 또는 PHONE 인증 완료 토큰으로 신규 회원을 생성합니다. "
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "400",
                    description = "요청 값 검증 실패, 필수 인증 토큰 누락 또는 인증한 이메일 불일치",
            ...
            @ApiResponse(
                    responseCode = "503",
                    description = "Redis 가입 인증 저장소 장애",
                    content = @Content(schema = @Schema(implementation = CommonErrorResponse.class))
            )
    })
