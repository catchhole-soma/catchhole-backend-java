...
public interface PhoneVerificationService {
     ...
     */
    PhoneVerificationSendResponse sendPhoneVerificationCode(String phoneNumber, String clientIp);

    /**
     * 인증번호를 원자적으로 검증하고 성공 시 회원가입용 토큰을 발급한다.
     * 같은 인증 흐름을 다시 확인하면 토큰이 유효한 동안 기존 토큰을 반환해 여러 토큰이 생기지 않게 한다.
     */
    PhoneVerificationConfirmResponse confirmPhoneVerificationCode(String verificationId, String verificationCode);

    /**
     * 회원가입의 DB 작업 전에 토큰과 연결된 검증 완료 번호를 조회한다.
     * 이 단계에서는 토큰을 소비하지 않아 이후 DB unique 검증과 저장을 먼저 수행할 수 있다.
     */
    String getVerifiedPhoneNumberBySignupToken(String signupToken);
